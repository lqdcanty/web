...
recevicetip(msg:boolean):void{
  ...
  ngOnInit() {
    ...
    this.http.post(environment.apiBase+"/pc/api/sysmsg",body,{headers:this.header})
    //this.http.get("http://localhost:4200/assets/data/sysmsg.json")
    .map(res=>res.json())
    .subscribe(data=>{
      if(data.httpCode==200){
        console.log(data.data.list);
        this.sysMes=data.data.list;
