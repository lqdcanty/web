...
@Component({
  selector: 'app-updata-news',
  templateUrl: './updata-news.component.html',
  styleUrls: ['./updata-news.component.css']
})
export class UpdataNewsComponent implements OnInit {
private updataMes:Array<any>;
...
recevicetip(msg:boolean):void{
  ...
  ngOnInit() {
    ...
    this.http.post(environment.apiBase+"/pc/api/updatemsg",body,{headers:this.header})
    //this.http.get("http://localhost:4200/assets/data/updatemsg.json")
    .map(res=>res.json())
    .subscribe(data=>{
      if(data.httpCode==200){
        console.log(data.data.list);
        this.updataMes=data.data.list;
