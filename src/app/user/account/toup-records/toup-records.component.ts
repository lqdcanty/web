...
@Component({
  selector: 'app-toup-records',
  templateUrl: './toup-records.component.html',
  styleUrls: ['./toup-records.component.css']
})
export class ToupRecordsComponent implements OnInit {
private rechargeList:Array<any>;
...
reviceLogin(msg:boolean){
  ...
  ngOnInit() {
    ...
    this.http.post(environment.apiBase+"/pc/api/recharge/logs",body,{headers:this.header})
    //this.http.get("http://localhost:4200/assets/data/recharge.json")
    .map(res=>res.json())
    .subscribe(data=>{
