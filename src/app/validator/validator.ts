...
export function passValueValidator(control: FormControl): any {
    // 获取密码输入框的值
    const pass = control.value;
    console.log('你输入的值:', pass);
    const regExp=/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,21}$/;
    const passValue=regExp.test(pass)
    return passValue ? null : { passValueValidator: { info: '密码是6-21位字母和数字组成' } };
}
/*export function careIdValidator(control: FormControl): any {
    // 获取密码输入框的值
    const careId = control.value;
    console.log('你输入的值:', careId);
