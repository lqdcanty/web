...
@Component({
  selector: 'app-draft-chapter',
  templateUrl: './draft-chapter.component.html',
  styleUrls: ['./draft-chapter.component.css']
})
export class DraftChapterComponent implements OnInit {
@Input() chapterParam;
@Input() btnShow;
@Output() outerDraftChapter =new EventEmitter<boolean>();
  ...
  ngOnInit() {
    //this.sanLogin();
      ...
      let body="chapterId="+this.chapterParam+"&userId="+userId;
  		this.http.post(environment.apiBase+"/pc/api/author/draft",body,{headers:this.header})
 ...
 setTime() {
  ...
  setTimeSend(){
      if($("#time").val()){
        ...
        let userId=this.getCookie("userId");
        let chapterId=-1;
        let draftId=this.chapterParam?this.chapterParam:-1;
            ...
            if(this.chapter.get('content').value.length>1000){
              ...
              .subscribe(data=>{
                if(data.httpCode==100){
                  ...
                  setTimeout(() => {
                    //this.tipLayerBoolean=false;
                    this.outerDraftChapter.emit(true);
                  }, 1500);
  ...
  immediateClick():void{
    console.log(this.timeValue,"shijain ")
    let userId=this.getCookie("userId");
    if(userId){
  ...
  saveChapterFunction():void{
    ...
    if(userId){
        ...
        let draftId=this.chapterParam?this.chapterParam:-1;
        let body="userId="+userId+"&bookId="+this.bookId+"&chapterName="+chapter.chapterName+"&content="+chapter.content+"&remark="+chapter.remark+"&chapterId="+chapterId+"&draftId="+draftId;
        this.http.post(environment.apiBase+"/pc/api/author/tmp/save",body,{headers:this.header})
        .map(res=>res.json())
        .subscribe(data=>{
          if(data.httpCode==100){
  ...
  reset(){
    let userId=this.getCookie("userId");
    let draftId=this.chapterParam?this.chapterParam:-1;
    let body="userId="+userId+"&bookId="+this.bookId+"&draftId="+draftId;
    this.http.post(environment.apiBase+"/pc/api/author/draft/del",body,{headers:this.header})
    .map(res=>res.json())
    .subscribe(data=>{
